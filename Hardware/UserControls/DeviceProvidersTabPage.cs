...
{
    internal class DeviceProvidersTabPage : UserControl
    {
        ...
        private void UpdateContextMenu(TreeView treeView, TreeNode selectedNode, object selectedEntity)
        {
            ...
            ToolStripMenuItem menuCreateDeviceProvider = new("Добавить поставщика устройства");
            menuCreateDeviceProvider.Click += (sender, e) => { CreateEntity(typeof(DeviceType)); };
            ...
            if (selectedEntity is DeviceProvider deviceProvider)
            {
                menuCreate.Visible = false;
                menuUpdate.Text = "Изменить поставщика устройства";
                menuUpdate.Click += (sender, e) => { UpdateEntity(deviceProvider); };
                menuDelete.Text = "Удалить поставщика устройства";
                menuDelete.Click += (sender, e) => { DeleteEntity(deviceProvider); };
            }
            ...
            contextMenu.Items.Add(menuCreateDeviceProvider);
            contextMenu.Items.Add(new ToolStripSeparator());
        ...
        private void SwitchMoveButtons()
        {
            ...
            if (selectedEntityLeft is Device device && selectedEntityRight is DeviceProvider deviceProvider)
                if (device.DeviceProviderId != deviceProvider.Id)
                {
                    moveEntityToRightBtn.Enabled = true;
                    moveEntityToRightBtn.Text = $"{device.Serial} >>> {deviceProvider.Name}";
                    moveEntityToRightBtn.Click += (sender, e) => { UpdateEntity(device, deviceProvider); };
                }

            if (selectedEntityRight is Device device1 && selectedEntityLeft is DeviceProvider deviceProvider1)
                if (device1.DeviceProviderId != deviceProvider1.Id)
                {
                    moveEntityToLeftBtn.Enabled = true;
                    moveEntityToLeftBtn.Text = $"{deviceProvider1.Name} <<< {device1.Serial}";
                    moveEntityToLeftBtn.Click += (sender, e) => { UpdateEntity(device1, deviceProvider1); };
                }
        ...
        private async void LoadData(TreeView treeView, string filter)
        {
            ...
            using ApplicationContext context = new ApplicationContextFactory(configManager).CreateDbContext();
            List<DeviceProvider> deviceProviders = await context.ReadDeviceProviders(filter);
            foreach (DeviceProvider deviceProvider in deviceProviders)
            {
                TreeNode deviceProviderNode = new()
                {
                    Text = deviceProvider.Name,
                    Tag = deviceProvider,
                    ImageKey = "deviceProvider",
                    ToolTipText = $"Поставщик устройства: {deviceProvider.Name}"
                };

                foreach (Device device in deviceProvider.Devices.OrderBy(d => d.Inventory).ThenBy(d => d.DeviceName.Name))
                {
                    ...
                    deviceProviderNode.Nodes.Add(deviceNode);
                }

                treeView.Nodes.Add(deviceProviderNode);
            }
